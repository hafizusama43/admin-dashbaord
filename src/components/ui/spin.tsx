...
const spinVariants = cva("flex items-center justify-center animate-spin rounded-full", {
  variants: {
    variant: {
...
const Spin = React.forwardRef<HTMLDivElement, SpinProps>(
  ({ className, variant, size, ...props }, ref) => {
    return <div className={cn(spinVariants({ variant, size, className }))} ref={ref} {...props} />;
  }
);
